...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("query one mut from 10,000 entities", |b| {
        let mut manager = setup_manager::<10_000>();
        manager.update();

        b.iter(|| {
            let my_comp = manager.query_entities_component_one_mut::<MyComponent>();
            assert!(my_comp.is_some());
        });
    });
    ...
    c.bench_function("query one from 10,000 entities", |b| {
        let mut manager = setup_manager::<10_000>();
        manager.update();

        b.iter(|| {
            let my_comp = manager.query_entities_component_one::<MyComponent>();
            assert!(my_comp.is_some());
        });
    });
    ...
    c.bench_function("query one multiple comps from 10,000 entities", |b| {
        let mut manager = setup_manager::<10_000>();
        manager.update();

        b.iter(|| {
            let my_comp = manager.query_entities_components_one::<(MyComponent, ComponentC)>();
            assert!(my_comp.is_some());
        });
    });
